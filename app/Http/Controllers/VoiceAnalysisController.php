...
class VoiceAnalysisController extends Controller
{
    ...
    public function voiceRecord(Request $request)
    {
        ...
        \Log::debug("check in record");
        \Log::debug($request->all());

        $pubPath = public_path('audio\\');
        ...
        if ($recordDuration > $originDuration) {
            $tmp = $recordDuration;
            $recordDuration = $originDuration;
            $originDuration = $tmp;
        }

        $durationDistance = (int) ((($originDuration - ($originDuration - $recordDuration)) / $originDuration) * 100);
    ...
    public function voiceExtraction(Request $request)
    {
        ...
        $filePath = public_path('storage\\video\\' . $fileName);
        ...
        $path = 'audio\\' . $fileName . '_origin.wav';
